"""
Train v3 model with psychology theory data (12000 examples).
"""
import os
...
device = "cuda"
OUT_DIR = os.path.join(ROOT_DIR, "checkpoints", "qwen_psych_v3")
...
texts = []
with open("data_psychology/psych_v3_train.jsonl") as f:
    for line in f:
...
for epoch in range(5):
    ...
    for i in range(0, n, B):
        if step >= MAX_STEPS: break
        ids = enc.input_ids[idx[i:i+B]].to(device)
        attn = enc.attention_mask[idx[i:i+B]].to(device)
